...
一个 AI 驱动的自驾游规划工具。用户通过对话与 AI 交互，AI 研究路线走廊、推荐沿途景点，并维护一个**实时更新的行程单**。用户无需填写表单——直接对话，系统同步更新三面板仪表盘。

### 核心设计原则

- **行程始终实时** — 从第一个完整输入开始就存在，每次对话后重新计算
- **地图是决策工具** — 可视化路线检查替代手动推理
- **AI 负责约束** — 用户设定一次（驾驶限制、预算），AI 全程执行
- **级联效果立即可见** — 添加或删除景点会立即重塑整个行程
...
| 层 | 职责 | 实现 |
|----|------|------|
| Layer 1 | 自动推荐沿途景点 | 高德多边形搜索 API（1次调用） |
| Layer 2 | 用户补充特定地点 | 表单输入 + 高德搜索 |
| Layer 3 | AI 个性化推荐 | 网络检索 + 智能判断 |
